...
func TestStylish(t *testing.T) {
	testData := []struct {
			...
			diff: compare.DiffMap{
				"group": {
					...
					Children: compare.DiffMap{
						"key": {Status: compare.CodeUnchanged, OldValue: "value"},
					},
				},
			},
			want: "{\n" +
		...
		},
		{
			...
			diff: compare.DiffMap{
				"group": {
					Status: compare.CodeUnchanged,
					Children: compare.DiffMap{
						"foo": {Status: compare.CodeAdded, NewValue: "bar"},
						"baz": {Status: compare.CodeRemoved, OldValue: "bas"},
					},
				},
			},
			want: "{\n" +
		...
		},
		{
			name: "raw map value",
			diff: compare.DiffMap{
				"group": {Status: compare.CodeRemoved, OldValue: map[string]any{"key": "value"}},
			},
			want: "{\n" +
				"  - group: {\n" +
				"        key: value\n" +
